...
func makeManifest(_ generation: Int64, _ title: String) -> SessionManifest {
    SessionManifest(schemaVersion: 1, generation: generation, savedAt: Date(), windows: [SessionWindowRecord(windowID: "win-1", frameX: nil, frameY: nil, frameWidth: nil, frameHeight: nil, workspacePath: nil, sidebarVisible: nil, sidebarTab: nil, sidebarWidth: nil, outlineDetached: nil, outlineWidth: nil, statusBarVisible: nil, tabOrder: ["tab-1"], activeTabID: "tab-1", tabs: [SessionTabRecord(tabID: "tab-1", path: nil, title: title, untitledSequence: 1, isDirty: true, revision: 1, encoding: "UTF-8", newLine: "LF", fingerprintModificationSeconds: nil, fingerprintSize: nil, cursorPosition: nil, selectionAnchor: nil, selectionHead: nil, scrollTop: nil, snapshotFileName: "snap-tab-1.md")])])
}
let name = try store.writeSnapshot(tabID: "tab-1", content: "# hi", revision: 1, encoding: "UTF-8", newLine: "LF")
...
try store.commit(manifest: makeManifest(3, "gen3")); try store.writeSnapshot(tabID: "tab-9", content: "orphan", revision: 1, encoding: "UTF-8", newLine: "LF")
store.pruneOrphanSnapshots(keeping: store.loadLatest().manifest!)
expect(store.readSnapshot(fileName: "snap-tab-9.md") == nil, "orphan removed")
expect(store.readSnapshot(fileName: "snap-tab-1.md") == "# hi", "referenced snapshot kept")
print("PASS")
