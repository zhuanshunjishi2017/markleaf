...
expect(EditorCommandRouter.scope(for: "restartEditor") == .windowLevel, "restartEditor is window scope")
expect(EditorCommandRouter.scope(for: "learnMarkdown") == .app, "learnMarkdown is app scope")
for command in ["toggleHighlight", "insertAlertNote", "insertAlertTip", "insertAlertImportant",
                "insertAlertWarning", "insertAlertCaution", "showFrontMatter",
                "setMathNumber", "copyHtml"] {
    expect(EditorCommandRouter.scope(for: command) == .document,
           "\(command) routes to the active document")
}
