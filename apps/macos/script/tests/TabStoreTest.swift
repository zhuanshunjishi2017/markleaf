...
// 跨窗口拖拽并入：按目标下标插入，越界自动收敛，并默认激活新标签。
let insertStore = TabStore()
insertStore.append(makeTab("/tmp/one.md"), activate: false)
insertStore.append(makeTab("/tmp/two.md"), activate: false)
let moved = makeTab("/tmp/moved.md")
insertStore.insert(moved, at: 1)
expect(insertStore.tabs.map(\.title) == ["one.md", "moved.md", "two.md"], "insert places the tab at the drop index")
expect(insertStore.activeTabID == moved.tabID, "insert activates the moved tab by default")
let appended = makeTab("/tmp/last.md")
insertStore.insert(appended, at: 99)
expect(insertStore.tabs.map(\.title) == ["one.md", "moved.md", "two.md", "last.md"], "an out-of-range index appends at the end")
print("PASS")
