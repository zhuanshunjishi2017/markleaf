...
BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/markleaf-export-settings-test.XXXXXX")"
trap 'rm -rf "$BUILD_DIR"' EXIT
SDK_PATH="${SDKROOT:-$(xcrun --sdk macosx --show-sdk-path)}"
cp "$ROOT_DIR/script/tests/PersistedExportSettingsTest.swift" "$BUILD_DIR/main.swift"
swiftc -sdk "$SDK_PATH" -module-cache-path "$BUILD_DIR/module-cache" \
  ...
  "$BUILD_DIR/main.swift" -o "$BUILD_DIR/policy-test"
