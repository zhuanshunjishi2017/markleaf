...
BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/markleaf-window-session-open-routing-test.XXXXXX")"
...
swiftc -sdk "$SDK_PATH" -module-cache-path "$BUILD_DIR/module-cache" \
  ...
  -o "$BUILD_DIR/window-session-open-routing-test"
"$BUILD_DIR/window-session-open-routing-test"
