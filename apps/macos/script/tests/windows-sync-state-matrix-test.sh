...
# 源码模式在专注/打字机之后加入禁用矩阵，因此当前实现是三者取反的组合。
require "$POLICY" '!isSourceMode && !isReadOnly && !isPlainText' 'editor modes must be disabled for source mode, read-only and plain text'
