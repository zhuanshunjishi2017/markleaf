...
| 公式、Mermaid、代码 | 上下文菜单、选中后再次点击展开源码、代码语言按钮 | 复用原有渲染、数学辅助、源码控件，编辑结果进入 VS Code 文档历史。 |
...
| 剪贴板 | 普通复制/剪切/粘贴及“编辑”菜单 | 选区 Markdown/HTML 源码/纯文本复制；可视模式的普通文本和纯文本粘贴按共享内核规则解析 Markdown，显示转换、降级原因和失败；源码编辑保留字面文本。 |
...
2026-09-11 架构收敛轮只进行类型检查、编译和打包，未执行项目测试。迁移和新增测试需要后续统一运行，不把本轮编译成功视为这些测试通过。
