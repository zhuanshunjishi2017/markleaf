# VS Code 功能对应说明（0.2.5）

本表按当前共享编辑器、macOS 菜单和 Windows 宿主已有职责核对。编辑功能复用共享内核，文件和窗口管理由 VS Code 提供；实施范围停在导出与打印之前。表中的接入情况指源码链路和入口，实际平台交互仍需安装验收。
...
| 阅读和排版 | “视图”、偏好和扩展设置 | 九种排版、十九种配色或跟随 VS Code，字体/宽度/行高、中西文间距、缩放、专注、打字机、状态栏、自动隐藏滚动条。 |
...
| PDF、HTML 文件、长图导出、打印 | 后续阶段 | 本阶段没有注册导出或打印命令。复制选区为 HTML 和图片另存为属于现有编辑/资源操作。 |
...
VS Code 前端和扩展宿主分别编译，并生成 VSIX；共享原生前端单独构建。实际 VS Code 窗口中的安装、输入法、快捷键、图片拖放，以及 Windows/Linux/SSH/WSL 行为需要对应环境验收。本阶段没有构建原生 macOS/Windows 应用，也没有执行导出或打印验收。
