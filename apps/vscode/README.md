...
本地构建得到 `artifacts/markleaf-vscode-0.1.1.vsix` 后，在 VS Code 的扩展菜单选择 **Install from VSIX…**，或执行：

```bash
code --install-extension artifacts/markleaf-vscode-0.1.1.vsix
```
...
- **格式**：工具栏支持粗体、斜体、下划线、删除线、高亮和行内代码；“格式…”按文字、段落、列表、提示框、表格、公式与图表分组，支持按名称搜索。也可通过编辑区域右键菜单或命令面板的 **MarkLeaf: Format / Paragraph Actions** 打开。
- **段落**：支持清除格式、标题升降级、在当前段落前后插入、复制/删除当前段落、列表增加/减少缩进，以及 NOTE、TIP、IMPORTANT、WARNING、CAUTION 五种提示框。操作沿用共享编辑内核和 VS Code 文档历史；不适用于当前选区时会显示提示。
- **菜单选区**：打开格式菜单、链接或图片地址输入框后，操作仍应用到打开菜单时的选区。如果等待期间文档已被编辑或源码更新，会提示重新选择，避免使用过期位置。
...
下划线兼容读取边界明确的 `++文字++`，内侧首尾不能是空白，外侧不能与英文字母、数字、下划线或其他加号粘连。`C++`、`C++17` 和普通递增运算符保留为文本。可视化编辑后的下划线保存为 `<u>…</u>`，避免与字面加号或单词内部选区产生歧义；下划线中的连续空格和其他行内格式也会保留。这一修复位于共享编辑器，原生版和 VS Code 扩展使用相同规则。
...
- 已实现：基础文档接入、源码/渲染切换、文字格式与段落操作；0.1.1 修复了 `C++` 下划线误解析。
- 后续组：格式刷与段落操作柄；表格、脚注、公式和图表的完整上下文操作；图片资源处理；查找替换、大纲、排版主题和偏好；导出与打印。
- 每组单独记录验证结果。构建和自动测试不代替实际安装后的界面验收；本阶段测试变更记录位于源码的 `apps/vscode/docs/TCR-001-rendering-formatting.md`。
