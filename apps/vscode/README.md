...
0.2.0 接入文字与段落操作、格式刷、表格、脚注、公式、图表、图片资源、查找替换、大纲及阅读偏好。导出与打印留待后续阶段。

0.2.1 修复源码 / 渲染双向快捷键及查找、替换、纯文本粘贴的焦点条件，保留编辑器已有格式快捷键。

0.2.2 修复工具栏下拉菜单的关闭行为：点击外部、按 Escape、切换菜单或焦点离开插件时收起。公式与 Mermaid 源码浮层使用不透明底色，靠近窗口边缘时调整位置；公式符号面板在下方空间不足时向上展开，并适配窄窗口。

0.2.3 接入 VS Code 专用的格式快捷键设置：录键、清除、恢复默认、冲突提示及菜单键位显示。可直接通过自定义键位打开公式辅助，沿用共享内核的编辑行为。

0.2.4 将 VS Code 中的公式与图表源码面板始终定位在对应内容下方，随文档滚动移出视野，不再根据视口空间上下跳转或贴边。快捷键面板在当前窗口尚未注册配置时明确提示重新加载窗口，并保留保存失败状态。
...
通过“视图 → 排版与设置”选择常用选项，或在 VS Code 设置中搜索 `@ext:markleaf.markleaf`。设置遵循已有的用户、工作区或工作区文件夹作用域。
