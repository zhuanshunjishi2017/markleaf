...
final class PreferencesWindowController: NSWindowController {
    ...
    private let autoSaveCheck = NSButton(checkboxWithTitle: "自动保存文件", target: nil, action: nil)
    private let snapshotIntervalField = NSTextField(string: "30")
    private let newLinePopup = NSPopUpButton()
    private let recordRecentFilesCheck = NSButton(checkboxWithTitle: "记录最近文件", target: nil, action: nil)
    private let recordRecentFoldersCheck = NSButton(checkboxWithTitle: "记录最近文件夹", target: nil, action: nil)
    ...
    private let topMostCheck = NSButton(checkboxWithTitle: "将窗口置于顶层", target: nil, action: nil)

    // 通用
    private let languagePopup = NSPopUpButton()
    private let associateMDCheck = NSButton(checkboxWithTitle: "Markdown 文件 (.md / .markdown)", target: nil, action: nil)
    private let associateTextCheck = NSButton(checkboxWithTitle: "纯文本文件 (.txt)", target: nil, action: nil)
    ...
    private let useRelativePathsCheck = NSButton(checkboxWithTitle: "在可用时使用相对路径", target: nil, action: nil)
    ...
    init(styles: [StyleDefinition], themes: [ColorThemeInfo]) {
        ...
        // ---- 控件初值 ----
        startupPopup.addItems(withTitles: ["新建文档", "打开上次工作区", "打开上次工作区及文件"])
        ...
        // i18n 尚未实现：与 Windows 版一致，仅提供「简体中文」占位（不置灰）
        languagePopup.addItems(withTitles: ["简体中文"])
        ...
        clipboardImagePopup.addItems(withTitles: ["保存到默认目录", "复制到文档资源"])
        clipboardImagePopup.selectItem(at: settings.clipboardImageHandling == "copyToAssets" ? 1 : 0)
        fileImagePopup.addItems(withTitles: ["引用原位置", "复制到文档资源"])
        ...
        imageDirectoryField.placeholderString = "默认 ~/Pictures/MarkLeaf"
        ...
        let tabs: [(String, String, NSView)] = [
            ("文件", "doc.text", filePage()),
            ("编辑器", "textformat", editorPage()),
            ("外观", "paintbrush", appearancePage()),
            ("通用", "gearshape", generalPage()),
            ("图片", "photo", imagePage()),
        ]
    ...
    private func filePage() -> NSView {
        formPage(rows: [
            ...
            .field("", linkButton("清除历史记录…", #selector(clearHistory))),
        ])
    }
    ...
    private func editorPage() -> NSView {
        formPage(rows: [
            .header("可视化"),
            .field("基础行高", lineHeightField),
            .field("基础字号", fontSizeField),
            .field("最大内容宽度", fieldRow(maxWidthField, unit: "px")),
            .header("源码模式"),
            .field("基础字号", sourceFontSizeField),
            .field("默认缩进宽度", sourceIndentField),
            .hint("部分排版设置可能由当前的排版样式接管，可到「外观」更改。"),
        ])
    }
    ...
    private func appearancePage() -> NSView {
        formPage(rows: [
            .header("文档外观"),
            .field("排版样式", stylePopup),
            .field("颜色主题", themePopup),
            .field("", linkButton("打开主题文件夹…", #selector(openThemeFolder))),
            .header("缩放视图"),
            .field("设置缩放", zoomPopup),
            .field("", linkButton("重置为 100%", #selector(resetZoom))),
    ...
    private func generalPage() -> NSView {
        formPage(rows: [
            ...
            .header("日志管理"),
            .field("", linkButton("打开日志目录…", #selector(openLogFolder))),
            .field("", linkButton("清除日志", #selector(clearLogs))),
            .header("高级"),
            .field("", linkButton("配置 JSON 文件…", #selector(revealSettingsJSON))),
            .field("", linkButton("恢复默认设置…", #selector(resetAll))),
        ])
    }
    ...
    private func imagePage() -> NSView {
        formPage(rows: [
            .header("剪贴板图片"),
            .field("处理方式", clipboardImagePopup),
            .header("文件图片"),
            .field("处理方式", fileImagePopup),
            .header("默认目录"),
            .field("", imageDirectoryField),
            .field("", linkButton("浏览…", #selector(browseImageDirectory))),
            .field("", useRelativePathsCheck),
            .field("", prefixDotSlashCheck),
            .hint("相对路径仅在文档已保存到本地时生效；文档未保存时“复制到文档资源”会回退到默认目录。"),
        ])
    }
    ...
    @objc private func resetAll() {
        let alert = NSAlert()
        alert.messageText = "重置所有设置为默认值？"
        alert.informativeText = "此操作会恢复默认设置，包括外观、缩放与图片设置。"
