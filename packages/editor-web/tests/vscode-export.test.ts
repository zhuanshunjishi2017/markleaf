...
import { createEditor, getMarkdown, setHostImageResolver } from '../src/editor'
...
import { generateExportHtml } from '../src/export-html'
...
describe('VS Code complete-document exports', () => {
  it('renders a full snapshot with local image references, KaTeX and minimal typography without editing the visible document', async () => {
    ...
    try {
      ...
      expect(html.querySelector('#export-root')?.classList.contains('markleaf-style-minimal')).toBe(true)
  ...
  it('keeps selected typography dependencies, page settings and localized labels in standalone HTML', async () => {
    ...
    expect(html.querySelector('#export-root')?.className).toContain('markleaf-style-print markleaf-style-print-double')
