 ...
 * - 三个产品（macOS、Windows、VS Code 扩展）均只经由本文件消费内核，
 *   不得深入 import 内核内部模块。
 *
 * 不属于内核、由各宿主适配层自行持有：
 ...
 *
 * 分层约束：内核依赖 DOM，只能在 webview / WebView2 / WKWebView 中运行。
 * editor-state.ts 是位于内核之下的纯类型契约层，不依赖 DOM，被扩展进程等
 * 无 DOM 环境直接引用；此处一并 re-export 只为 webview 侧取用方便，
 * 无 DOM 的宿主进程必须直接 import './editor-state'，不得经由本文件。
 *
