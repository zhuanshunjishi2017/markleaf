...
Windows のネイティブ版は PDF、HTML、PNG/JPG の縦長画像、印刷に対応し、macOS 版は PDF、HTML、システム印刷に対応します。VS Code 拡張機能にも PDF、単独 HTML、PNG/JPG 画像、プレビュー、ブラウザー印刷を追加しました。既定の組版は MarkLeaf の「Web · 極簡」です。PDF では用紙、向き、余白、ヘッダー・フッター、ページ番号を設定でき、縦長画像は連番ファイルに分割できます。HTML ファイルの出力以外は `puppeteer-core` でインストール済みの Chrome/Edge を使います。ブラウザーを同梱・ダウンロードしません。
