...
阶段 6 已支持表格、任务列表和图片。图片使用本机绝对路径；剪贴板位图写入应用目录下的 `Cache/ClipboardImages`。阶段 6.5 已补全状态栏和编辑区原生右键菜单。阶段 7 已加入异步工作区文件树、最近项目、基本文件操作、AST 大纲同步和 DPI Shell 文件图标。阶段 8 已加入当前文档查找替换、CodeMirror 6 源码模式和彼此隔离的多窗口实例。
...
- 自动恢复和会话恢复属于后续阶段；当前不可执行的菜单项会保持禁用。
